    ...
    <section className="bg-panel border border-panel-edge rounded-sm flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-auto">
        ...
        {pending.map((s) => {
          ...
          const canApprove = !disabled && (arm?.armed ?? false);
          return (
            <article
              key={s.id}
            ...
            >
              ...
              <div className="mt-2 flex items-center gap-1.5 pl-3.5">
                <button
                  type="button"
                  onClick={() => broadcastApi.approveAutomation(s.id)}
                  disabled={!canApprove}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-sm text-[11px] font-semibold uppercase tracking-wider bg-status-ok/15 text-status-ok border border-status-ok/40 hover:bg-status-ok/25 disabled:opacity-40 disabled:cursor-not-allowed"
                >
                ...
                <button
                  type="button"
                  onClick={() => broadcastApi.dismissAutomation(s.id)}
                  className="inline-flex items-center gap-1 px-2 py-1 rounded-sm text-[11px] font-semibold uppercase tracking-wider bg-panel-elev text-foreground/80 border border-panel-edge hover:bg-panel-edge"
                >
                  <X className="h-3 w-3" />
                  Dismiss
                </button>
